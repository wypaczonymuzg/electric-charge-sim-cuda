...
void runImageFilters(TColor *d_dst) {
	switch (g_Kernel) {
	case 0:
		cuda_sim_hardcoded(d_dst, imageW, imageH, sh_size);
		//cuda_sim(d_dst, d_array, imageW, imageH, num_of_charges ,sh_size);
		break;
...
//float* array;
	...
	printf(
			"choose simulation : \n1 : Random 200 charges\n2 : 100 charges evenly distributed\n3 : 2 charges in middle\n");
	scanf("%d", &n);
	switch (n) {
	case 1:
		num_of_charges = 256;
		...
		for (int i = 0; i < num_of_charges * 3; i += 3) {
	...
	case 2:
		...
		sh_size = sizeof(float) * num_of_charges * 3;
		printf("number of charges = %d \t sh_size = %d \n",num_of_charges,sh_size);
		h_array = (float*) malloc(sizeof(float) * num_of_charges * 3);
		for (int j = 0; j < num_of_charges / 10; j++) {
			for (int i = 0; i < num_of_charges / 10; i++) {
				int x = j*10+i;
				h_array[x*3] = i*100+150;
				h_array[x*3+1] = j*70+100;
				h_array[x*3+2] = 100000;
				printf("i : %d\t x : %f\t y : %f\t q : %f\n ", x, h_array[x*3],
						h_array[x*3+ 1], h_array[x*3 + 2]);
			}
		}
		g_Kernel = 2;
		break;
