...
void runImageFilters(TColor *d_dst,int *array,int size) {
	switch (g_Kernel) {
	case 0:
		cuda_Copy(d_dst, imageW, imageH, array,size );
		break;
...
void displayFunc(void) {
	...
	runImageFilters(d_dst,chargesArrayDevice,size);
...
void shutDown(unsigned char k, int /*x*/, int /*y*/) {
	...
	case 'Q':
		...
		cudaMemcpy(array_host_2, array_device_2, sizeof(int) * 6,
						cudaMemcpyDeviceToHost);

		cudaFree (array_device_2);
		checkCudaErrors(CUDA_FreeArray());
		for(int i=0;i<6;i++){
			printf("%d :\t%d ",i,array_host_2[i]);
		}
...
int main(int argc, char **argv) {
	...
	pArgc = &argc;
	pArgv = argv;
	int chargesArrayHost[12] = { 600, 600, 1000000, 250, 250, 1000000,800,600,1000000,435,543,1000000 };
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "file")) {
		...
		if (checkCmdLineFlag(argc, (const char **) argv, "kernel")) {
			kernel = getCmdLineArgumentInt(argc, (const char **) argv,
					"kernel");
		}
		...
		if (image_path == NULL) {
			printf(
					"imageDenoisingGL was unable to find and load image file <portrait_noise.bmp>.\nExiting...\n");
			exit(EXIT_FAILURE);
		}
		...
		size=0;
		chargesArrayDevice = new int[6];
		for (int i = 0; i < 12; i++) {
			chargesArrayDevice[i] = chargesArrayHost[i];
			size++;
		}
		cudaMalloc((void**) &chargesArrayDevice, sizeof(int) * 6);
		cudaMemcpy(chargesArrayDevice, chargesArrayHost, sizeof(int) * 6,
